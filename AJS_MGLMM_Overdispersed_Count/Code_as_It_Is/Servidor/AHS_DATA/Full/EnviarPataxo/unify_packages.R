...
TMB_summary <- function(fixef, adreport, nresp=5){
...
sv_sd <- function(obj, fit, time, units=60*60, nr, model, method, threads){
  rep <- sdreport(obj)
  rep_fe <- summary(rep, "fixed")
  rep_cor <- summary(rep, "report")
  all_estimates <- TMB_summary(fixef = rep_fe, adreport = rep_cor, nresp = nr)
