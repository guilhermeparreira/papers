...
###############################
# Retroalimentacao no NLMINB - DIDN'T WORK
###############################
parameters <- list(U = U,
                   beta = estimates_negbin_2$beta, # Didn't work
                   rho = estimates_negbin_2$rho,
...
# save.image("Negbin_full.RData")
# # load("Negbin_full.RData")
# estimates_poisson_s2 <- sv_sd(obj = obj, fit = fit_NLB2, time = a2, nr = nr, 
...
# ###### Fourth time
# # 1806,228
# estimates_negbin_2 <- sv(fit = fit_NLB2, time = a2, n_betas = n_params, nr = nr,
#                          model = model, method = "BFGS", threads = threads)
# parameters <- list(beta = estimates_negbin_2$beta,
#                    U = U,
#                    rho = estimates_negbin_2$rho,
...
#                  random = c("U"))
# 
# a2 <- system.time(fit_NLB3 <- optim(obj1$par, obj1$fn, obj1$gr,
#                                     method = "BFGS",
#                                     control = list(maxit = 1e8,
#                                                    abstol = 1e-04, reltol = 1e-04)))
# # fit_NLB2
...
# ###### Fifth time
# # 1837,831
# estimates_negbin_2 <- sv(fit = fit_NLB3, time = a2, n_betas = n_params, nr = nr,
#                          model = model, method = "BFGS", threads = threads)
# parameters <- list(beta = estimates_negbin_2$beta,
#                    U = U,
#                    rho = estimates_negbin_2$rho,
...
#                   random = c("U"))
# 
# a2 <- system.time(fit_NLB4 <- optim(obj1$par, obj1$fn, obj1$gr,
#                                     method = "BFGS",
#                                     control = list(maxit = 1e8,
#                                                    abstol = 1e-04, reltol = 1e-04)))
# # fit_NLB2
# fit_NLB4[2]
# estimates_poisson_s2 <- sv_sd(obj = obj1, fit = fit_NLB4, time = a2, nr = nr, 
#                               model = model, method = "bfgs", threads = threads)
# 
# save.image("Negbin_full.RData")